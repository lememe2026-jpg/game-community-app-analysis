...
- 在 `xhh_negative_issue_summary.csv` 中确认 `negative_review_count` 为 338。
- 检查 `issue_category`、`negative_review_count`、`hit_count`、`percent`。
...
- 核对平台生态 / 玩家圈层 `keyword_count` 为 81，`core_clue_count` 为 38。
- 核对平台生态 / 竞品观察 `keyword_count` 为 65，`core_clue_count` 为 45。
...
- 2025-12-12 至 2025-12-16 连续高位窗口以正式报告日期表和图 6 为准。
- 公开仓库当前不单独提供完整每日明细表，`download_peak_dates.csv` 仅用于峰值日期复核。
